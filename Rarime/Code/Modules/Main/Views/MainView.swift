...
struct MainView: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .contact: EmptyView() // Contact is handled by sheet
            }
        ...
        .fullScreenCover(isPresented: $viewModel.isContactEmailSheetShown) {
            if MFMailComposeViewController.canSendMail() {
                CelestialsFeedbackMailView(isShowing: $viewModel.isContactEmailSheetShown)
            } else {
                // Fallback if mail is not available
                Text("Email client not available")
                    .onAppear {
                        viewModel.isContactEmailSheetShown = false
                    }
            }
        }
    }
